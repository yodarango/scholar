...
import { IdropdownOptions } from "../../helpers/english-bible-versions";
...
const [currVersionState, setCurrVersionState] = useState<IOpenVersionState>({id:'c315fa9f71d4af3a-01', initials: "KJV"});
...
const openVerChapPopup = (e: any)=>{
   setOpenVersionState(<PopupWrapper closeModal={()=> setOpenVersionState(false)} content= { 
   <div className={GeneralDropdownStyles.mainWrapper}>
   {dropdownOptions && dropdownOptions.map((option)=> ( <div key={option.id} 
   // pass the acronym and the bible version id
   onClick={()=>selectVersion({id: option.id, initials: option.abbreviation})} className={`std-text-block--widget ${readingCollageUnitStyles.optionUnit}`}>{option.name}</div>))}
</div>}/>)
}
...
const [currentChapter, setCurrentChapter] = useState<IcurrentChapter>({currChapterLoaded: <Chapter versionId={`de4e12af7f28f599-02`} chapterId="JHN.3"/>, currentReferenceSelected: "John 3"});
...
const openChapterPopup = () =>{
   setCurrentChapter({currChapterLoaded: false, currentReferenceSelected: false})
   setOpenBookState(<GetNewBook openGetNewChapterFunc ={openGetNewChapterFunc} closeModal={()=>setOpenBookState(false)} versionId={currVersionState.id}/>)
...
const openGetNewChapterFunc =(bookId: string) =>{
   setOpenChapterState(<GetNewChapter goBackModal={()=> setOpenChapterState(false)} openGetNewVerse={showSelectedBook} closeModal={()=>{setOpenBookState(false), setOpenChapterState(false)}} bookId={bookId} versionId={currVersionState.id}/>)
}
...
setCurrentChapter({currChapterLoaded: <Chapter chapterId={chapData.id} versionId={currVersionState.id}/>, currentReferenceSelected: chapData.reference})
      ...
      <div className={readingCollageUnitStyles.mainWrapper}>
         <div className={readingCollageUnitStyles.header}>
            <div className={readingCollageUnitStyles.versionChapterDropDownWrapper}>
               ...
               <div data-currentversion="JHN" className={readingCollageUnitStyles.chapterDropDownWrapper} onClick={openChapterPopup}>
                  <p className={`std-button_gradient-text`}>{currentChapter.currentReferenceSelected}</p>
               </div>
            </div>
         </div>
         <div className={readingCollageUnitStyles.currentChapterWrapper}>{currentChapter.currChapterLoaded}</div>
      </div>
      </>
   );
};
